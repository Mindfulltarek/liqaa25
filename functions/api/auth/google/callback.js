import { json, verifyOAuthState, requireDB, createSession, cookie, SESSION_COOKIE, SESSION_SECONDS } from '../_utils';
export async function onRequestGet({request,env}) { try {
 ...
 if(!code || !(await verifyOAuthState(state,env.GOOGLE_CLIENT_SECRET || ''))) return json({error:'INVALID_OAUTH_STATE'},400);
