import { json, requireDB, createSession, cookie, SESSION_COOKIE, SESSION_SECONDS } from '../_utils';
export async function onRequestGet({request,env}) { try {
 const url=new URL(request.url), code=url.searchParams.get('code'), state=url.searchParams.get('state'); const db=requireDB(env);
 if(!code || !state) return json({error:'INVALID_OAUTH_STATE'},400);
 const valid=await db.prepare('SELECT state FROM oauth_states WHERE state=? AND expires_at > CURRENT_TIMESTAMP').bind(state).first();
 if(!valid) return json({error:'INVALID_OAUTH_STATE'},400);
 ...
 let account=await db.prepare('SELECT user_id FROM accounts WHERE provider=? AND provider_account_id=?').bind('google',google.sub).first(); let userId;
 ...
 const session=await createSession(env,userId); return new Response(null,{status:302,headers:{Location:url.origin+'/dashboard.html','Set-Cookie':cookie(SESSION_COOKIE,session.id,SESSION_SECONDS)}});
